...
<h1><a href="/src/components/TodoListtest.vue">English</a></h1>
</div>
</div>
...
<!-- Style to the Box -->
  ...
  <div class="flex-grow w-4/6 border-2 border-purple-700 rounded ">
     
      <!-- To-Do Title -->
     
     <div class="bg-indigo-900  text-white text-center uppercase font-bold">Liste mit Todo's</div>
  ...
  <div class="flex justify-center bg-indigo-300 py-2">
    <form v-on:submit.prevent="addNewTodo">
      ...
      <button type="submit" class="ml-3 py-1 px-3 bg-indigo-800 text-white hover:bg-white hover:text-indigo-800 text-sm uppercase" @click="panel=true">To-Do addieren</button>
    </form>
  </div>
            ...
            <tr v-for="(todo, index) in todos" :class="[todo.completed?'bg-green-300':'bg-purple-200']">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="text-xs text-gray-900 text-left">
                  ...
                  <p v-if="todo.completed">Erledigt: {{todo.completed_at}}</p>
                </div>
              </td>
